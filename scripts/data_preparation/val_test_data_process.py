...
# 通过Bicubic获得低分辨率图像
def process_images_1(img_path_0, scale):
...
def main():
    ...
    if args.data_type == 'Test':
        ...
        output_dir = os.path.join(output_dir, args.dataset_name) # 对于测试集 需要多保存一个数据集名字的文件夹(考虑到会用多个不同的数据集进行测试)
    else:
    ...
    for i in range(0, len(imgs_list), 2):
        img_hr_0, img_lr_0 = process_images_1(imgs_list[i], scale = scale)
        img_hr_1, img_lr_1 = process_images_1(imgs_list[i + 1], scale = scale)
