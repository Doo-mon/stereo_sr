...
class NewImageRestorationModel(BaseModel):
    """Base Deblur model for single image deblur."""

    def __init__(self, opt):
        super(NewImageRestorationModel, self).__init__(opt)
...
class TwoStageStereoSRModel(BaseModel):
    ...
    def __init__(self, opt):
        super(TwoStageStereoSRModel, self).__init__(opt)

        # 前面进行一次单图超分 Transformer based
        self.net_t = define_network(deepcopy(opt['network_t']))
        self.net_t = self.model_to_device(self.net_t)

        # 后面再进行一次双目超分 CNN based
        self.net_g = define_network(deepcopy(opt['network_g']))
        self.net_g = self.model_to_device(self.net_g)
        ...
        if load_path_t is not None:
            self.load_network(self.net_t, load_path_t, self.opt['path'].get('strict_load_t', True), param_key=self.opt['path'].get('param_key', 'params'))

        # 加载预训练模型
        load_path_g = self.opt['path'].get('pretrain_network_g', None)
        if load_path_g is not None:
            self.load_network(self.net_g, load_path_g, self.opt['path'].get('strict_load_g', True), param_key=self.opt['path'].get('param_key', 'params'))
    ...
    def setup_optimizers(self):
        ...
        optim_type_t = train_opt['optim_t'].pop('type')

        if optim_type_g == 'Adam':
            self.optimizer_g = torch.optim.Adam([{'params': optim_params_g}],**train_opt['optim_g'])
        elif optim_type_g == 'SGD':
            self.optimizer_g = torch.optim.SGD(optim_params_g,**train_opt['optim_g'])
        elif optim_type_g == 'AdamW':
            self.optimizer_g = torch.optim.AdamW([{'params': optim_params_g}],**train_opt['optim_g'])
            pass
        else:
            raise NotImplementedError(
                f'optimizer {optim_type_g} is not supperted yet.')
        

        if optim_type_t == 'Adam':
            self.optimizer_t = torch.optim.Adam([{'params': optim_params_t}],**train_opt['optim_t'])
        elif optim_type_t == 'SGD':
            self.optimizer_t = torch.optim.SGD(optim_params_t,**train_opt['optim_t'])
        elif optim_type_t == 'AdamW':
            self.optimizer_t = torch.optim.AdamW([{'params': optim_params_t}],**train_opt['optim_t'])
            pass
        else:
            raise NotImplementedError(
                f'optimizer {optim_type_t} is not supperted yet.')
    ...
    def optimize_parameters(self, current_iter, tb_logger):
        ...
        # 输入先经过第一个阶段 然后再经过第二个阶段
        preds = self.net_t(self.lq)
        preds = self.net_g(preds)
        ...
        if use_grad_clip:
            torch.nn.utils.clip_grad_norm_(self.net_g.parameters(), 0.01)
            torch.nn.utils.clip_grad_norm_(self.net_t.parameters(), 0.01)
        self.optimizer_g.step()
